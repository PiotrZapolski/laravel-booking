...
<h1 style="font-size:24px;line-height:1.25;margin:0 0 12px;color:#0f172a;">
    {{ __('booking::booking.mail_confirmation_heading') }}
</h1>
<p style="margin:0 0 20px;color:#475569;font-size:15px;">
    {{ __('booking::booking.mail_confirmation_intro', ['title' => $eventType['title'] ?? '']) }}
</p>
...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"
    ...
    <tr>
        <td style="padding:14px 16px;background:#f8fafc;color:#475569;width:40%;">
            <strong>{{ __('booking::booking.mail_label_when') }}</strong>
        </td>
...
@if (!empty($payload['meet_link']))
    <p style="margin:0 0 24px;">
        <a href="{{ $payload['meet_link'] }}"
           style="display:inline-block;background:{{ $accent }};color:#ffffff;text-decoration:none;padding:12px 22px;border-radius:10px;font-weight:600;">
            {{ __('booking::booking.mail_btn_join') }}
        </a>
    </p>
@endif

@if (!empty($fieldRows))
    <h2 style="font-size:16px;margin:24px 0 10px;color:#0f172a;">{{ __('booking::booking.mail_your_details') }}</h2>
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"
...
@if (!empty($payload['reschedule_url']))
    <p style="margin:14px 0 0;font-size:14px;">
        {{ __('booking::booking.mail_need_change') }}
